package com.pppspringaopdemos.aspectjsupport.combindedpointcuts.aspect;
...
public class LoggingAspect {
    ...
    @Pointcut("com.pppspringaopdemos.aspectjsupport.combindedpointcuts.pointcuts.CommonPointcuts.inWebLayer() && execution(public * *(..))")
    public void publicWebLayerOperation() {}

    // 공통 포인트컷을 결합하여 서비스 레이어에서 트랜잭션이 필요한 메서드 포인트컷 정의
    // 다음 포인트컷 표현식의 의미를 부분별 설명:
    // com.pppspringaopdemos.aspectjsupport.combindedpointcuts.pointcuts.CommonPointcuts.businessService():
    //      이 부분은 CommonPointcuts 클래스에서 정의된 businessService 포인트컷을 참조.
    //      이 포인트컷은 com.pppspringaopdemos.aspectjsupport.combindedpointcuts.service 패키지 내의 모든 클래스에 있는 메서드를 대상으로 함.
    //
    // &&: 논리 AND 연산자입니다. 두 조건이 모두 참일 때만 포인트컷이 매칭됨.
    //
    // !com.pppspringaopdemos.aspectjsupport.combindedpointcuts.pointcuts.CommonPointcuts.inDataAccessLayer():
    //       이 부분은 CommonPointcuts 클래스에서 정의된 inDataAccessLayer 포인트컷을 참조함.
    //       이 포인트컷은 com.pppspringaopdemos.aspectjsupport.combindedpointcuts.dao 패키지 내의 모든 클래스에 있는 메서드를 대상으로 함. 
    ...
    //       즉 com.pppspringaopdemos.aspectjsupport.combindedpointcuts.service 패키지 내의 모든 클래스의 메서드 중에서,
    //   2. inDataAccessLayer 포인트컷에 의해 정의된 조건에 해당하지 않는 메서드,
    //       즉 com.pppspringaopdemos.aspectjsupport.combindedpointcuts.dao 패키지 내의 클래스에 속하지 않는 메서드.
    //
    //  간단히 말해, 서비스 레이어에 있는 메서드이지만, DAO(데이터 액세스) 레이어에 속하지 않는 메서드들을 대상으로 하는 포인트컷.
    //  이 조건에 해당하는 메서드들이 실행될 때 이 포인트컷이 매칭됨.
    @Pointcut("com.pppspringaopdemos.aspectjsupport.combindedpointcuts.pointcuts.CommonPointcuts.businessService() && "
    		+ "!com.pppspringaopdemos.aspectjsupport.combindedpointcuts.pointcuts.CommonPointcuts.inDataAccessLayer()")
